...
#AGVs can travel after each other with one time instance as separation.
#travel {k in TIME, i in NODES,j in NODES : k <= T}: X[i,j,k] <= edgeCap;

#Only one AGV can ocupy on edge at the time.
...
#complete travel before T, No need, enought with "sink" and "endFlow"
