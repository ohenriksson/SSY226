...
sink {k in TIME, (sr,sn) in LINK}:
sum {(i,sn) in ARCS: k-TAU[i,sn] >= 0} X[i,sn,TASK_ID[sr,sn],k-TAU[i,sn]] = Y[sn,TASK_ID[sr,sn],k];
...
source {k in TIME, (sr,sn) in LINK}:
sum {(sr,j) in ARCS} X[sr,j,TASK_ID[sr,sn],k] = Y[sr,TASK_ID[sr,sn],k];
...
# Don't know if needed.
# restrictToSink {k in TIME, (src,snk) in LINK, (i,snk) in ARCS, t in TASK: t != TASK_ID[src,snk]}: X[i,snk,t,k] = 0;
